...
class Students {
  ...
  getInfo() {
    return this.students
      .sort(
        (student1, student2) => new Student(student1).course - new Student(student2).course,
      )
      .map(
...
pColor.forEach((item) => {
  const ind = colors[Symbol.iterator]();
  item.addEventListener('click', changeColor(ind));
});
